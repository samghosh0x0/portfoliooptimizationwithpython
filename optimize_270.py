...
def opt_port270(risk_free_rate):
...
##    return(port_sharpe_ratio)
    ...
    def sharperatio(allocation_matrix):
        ...
        neg_port_sharpe_ratio = -1*((port_avg_returns -rfr270)/port_vol_returns) #intentionally creating negative sharpe ratio as we are minimizing 
...
##        handles.append(matplotlib.patches.Patch(color=plt.cm.Set3((i)/8.), label=l))
    ...
    plt.title("Optimal Portfolio Allocation (270 Day Returns)", fontsize=14)
    ...
    plt.savefig('Charts/optimal_portfolio/optimal_portfolio_270.png')
    plt.close()
    ##plt.show()
