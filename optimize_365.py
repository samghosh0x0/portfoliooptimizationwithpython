...
def opt_port365(risk_free_rate):
    
    
    
    rfr365 = (pow((1+risk_free_rate), (365/365)))-1     #changing 

    df = pd.read_csv('return_data/Return365_df.csv', parse_dates=['Date']) #chancing
...
##    return(port_sharpe_ratio)
    ...
    def sharperatio(allocation_matrix):
        ...
        neg_port_sharpe_ratio = -1*((port_avg_returns -rfr365)/port_vol_returns) #intentionally creating negative sharpe ratio as we are minimizing 
...
##            {'type':'eq', 'fun': con_real}]
    ...
    plt.title("Optimal Portfolio Allocation (365 Day Returns)", fontsize=14)
    ...
    plt.savefig('Charts/optimal_portfolio/optimal_portfolio_365.png')
    plt.close()
    ##plt.show()
