...
def main3(risk_free_rate):
    
    pd.options.display.float_format = '{:.2%}'.format

    rfr180 = (pow((1+risk_free_rate), (180/365)))-1

    df = pd.read_csv('return_data/Return180_df.csv', parse_dates=['Date'])
    ...
    sns.heatmap(corrMatrix, annot=True).set_title("Correlation Matrix for Rolling 180 Day Returns", fontsize=14)
    ##plt.show()
    plt.savefig('Charts/return180/corr_return180.png')
    plt.close()
    ...
    avg_returnminusriskfree = ((df_cor-rfr180).mean()).tolist()
    ...
    #for average
    avg = avg_df.plot(ax=axes[0], x="asset", y='average', kind='bar', title ="Average Rolling 180 Day Return",figsize=(15,10), fontsize=12, rot=0, color='g', legend=False, zorder=3)
    avg.grid(axis='y', zorder=0)
    avg.set(xlabel=None)
    avg.set(ylabel='Avg. Rol. 180-d Ret')
    ...
    #for volatility
    vol = vol_df.plot(ax=axes[1], x="asset", y='volatility', kind='bar', title ="Votality of Rolling 180 Day Return",figsize=(15,10), fontsize=12, rot=0, color='r', legend=False,  zorder=3)
    vol.grid(axis='y', zorder=0)
    vol.set(xlabel=None)
    vol.set(ylabel='Vol of Rol. 180-d Ret.')
    ...
    #for avg/vol
    avgvol = sharperatio_pd.plot(ax=axes[2], x="asset", y='sharperatio', kind='bar', title ="Sharpe Ratio:180 day Returns ,Risk Free Rate: "+str("{:.2f}".format((risk_free_rate*100)))+"%",figsize=(15,10), fontsize=12, rot=0, color='b',legend=False,  zorder=3)
    ...
    plt.savefig('Charts/return180/return_vol_comparison.png')
    plt.close()
    ...
    for j in range(len(assets)):
        ...
        plt.title(assets[j]+":Trend in 180 Day Return")
        plt.ylabel("180 Day Return")
        plt.grid()
        #plt.show()
        plt.savefig('Charts/return180/'+assets[j]+'trend_return180.png')
        plt.close()
    ...
    Normalized_trends =recreated_tlines_df.plot(x='Date_new', y=assets, kind="line", title ="Normalized Trend in Rolling 180-day Returns",figsize=(15,10), lw=4, fontsize=8, rot=0, legend=True, zorder=3)
    ...
    ##plt.show()
    plt.savefig('Charts/return180/normalized_trends180.png')
    plt.close()
