...
def main5(risk_free_rate):
    
    pd.options.display.float_format = '{:.2%}'.format
    
    rfr365 = (pow((1+risk_free_rate), (365/365)))-1    

    df = pd.read_csv('return_data/Return365_df.csv', parse_dates=['Date'])
    ...
    sns.heatmap(corrMatrix, annot=True).set_title("Correlation Matrix for Rolling 365 Day Returns", fontsize=14)
    ##plt.show()
    plt.savefig('Charts/return365/corr_return365.png')
    plt.close()
    ...
    avg_returnminusriskfree = ((df_cor-rfr365).mean()).tolist()
    ...
    # creating subplots for average returns, volatility and avg/vol

    fig, axes = plt.subplots(nrows=3, ncols=1)
    fig.tight_layout()

    #for average
    avg = avg_df.plot(ax=axes[0], x="asset", y='average', kind='bar', title ="Average Rolling 365 Day Return",figsize=(15,10), fontsize=12, rot=0, color='g', legend=False, zorder=3)
    avg.grid(axis='y', zorder=0)
    avg.set(xlabel=None)
    avg.set(ylabel='Avg. Rol. 365-d Ret')
    avg.yaxis.set_major_formatter(mtick.PercentFormatter())

    #creating data labels 
    for p1 in avg.patches:
        avg.annotate((str(round(p1.get_height(),1))+"%"), (p1.get_x() * 1.007, p1.get_height() * 1.05), color='black', fontsize=14)

    #for volatility
    vol = vol_df.plot(ax=axes[1], x="asset", y='volatility', kind='bar', title ="Votality of Rolling 365 Day Return",figsize=(15,10), fontsize=12, rot=0, color='r', legend=False,  zorder=3)
    vol.grid(axis='y', zorder=0)
    vol.set(xlabel=None)
    vol.set(ylabel='Vol of Rol. 365-d Ret.')
    vol.yaxis.set_major_formatter(mtick.PercentFormatter())

    #creating data labels 
    for p2 in vol.patches:
        vol.annotate((str(round(p2.get_height(),1))+"%"), (p2.get_x() * 1.007, p2.get_height() * 1.05), color='black', fontsize=14)


    #for avg/vol
    avgvol = sharperatio_pd.plot(ax=axes[2], x="asset", y='sharperatio', kind='bar', title ="Sharpe Ratio:365 day Returns ,Risk Free Rate: "+str("{:.2f}".format((risk_free_rate*100)))+"%",figsize=(15,10), fontsize=12, rot=0, color='b',legend=False,  zorder=3)
    ...
    for p3 in avgvol.patches:
        avgvol.annotate((str(round(p3.get_height(),1))), (p3.get_x() * 1.007, p3.get_height() * 1.05), color='black', fontsize=14)
        
    ##plt.show()

    plt.savefig('Charts/return365/return_vol_comparison.png')
    plt.close()
    ...
    for j in range(len(assets)):
        ...
        plt.title(assets[j]+":Trend in 365 Day Return")
        plt.ylabel("365 Day Return")
        plt.grid()
        #plt.show()
        plt.savefig('Charts/return365/'+assets[j]+'trend_return365.png')
        plt.close()
    ...
    Normalized_trends =recreated_tlines_df.plot(x='Date_new', y=assets, kind="line", title ="Normalized Trend in Rolling 365-day Returns",figsize=(15,10), lw=4, fontsize=8, rot=0, legend=True, zorder=3)
    ...
    ##plt.show()
    plt.savefig('Charts/return365/normalized_trends365.png')
    plt.close()
